...
class GradientDescent private[spark] (private var gradient: Gradient, private var updater: Updater)
  ...
  private var convergenceTol: Double = 1E-10
  ...
  //SAEED
  override def getAllWeights(): ListBuffer[(Long,BV[Double])] = {
    GradientDescent.Warray
  }
...
object GradientDescent extends Logging {
  ...
  val Warray = new ListBuffer[(Long,BV[Double])]
  /**
  ...
  def runMiniBatchSGD(
    ...
    while (!converged && i <= numIterations) {
      ...
      if(i%100 == 0 || i==1){
        ...
        Warray.append((System.currentTimeMillis(),weights.asBreeze))
      }
        ////////
