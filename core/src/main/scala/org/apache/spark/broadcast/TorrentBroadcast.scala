...
private[spark] class TorrentBroadcast[T: ClassTag](obj: T, id: Long)
  ...
  //scalastyle:off
  private var broadcastId = BroadcastBlockId(id)

  // we should be able to change the id too
  private var changedID = id
  ...
  // added by Saeed
  override protected def getID(): BroadcastBlockId = {
    broadcastId
  }

  //added by Saeed
  override protected def changeID(newID: Long): Unit = {
    changedID = newID
    broadcastId = BroadcastBlockId(newID)
  }
  ...
  private def writeBlocks(value: T): Int = {
    ...
    blocks.zipWithIndex.foreach { case (block, i) =>
      ...
      val pieceId = BroadcastBlockId(changedID, "piece" + i)
  ...
  private def readBlocks(): Array[BlockData] = {
    ...
    for (pid <- Random.shuffle(Seq.range(0, numBlocks))) {
      val pieceId = BroadcastBlockId(changedID, "piece" + pid)
  ...
  override protected def doUnpersist(blocking: Boolean) {
    TorrentBroadcast.unpersist(changedID, removeFromDriver = false, blocking)
  }
  ...
  override protected def doDestroy(blocking: Boolean) {
    TorrentBroadcast.unpersist(changedID, removeFromDriver = true, blocking)
  }
